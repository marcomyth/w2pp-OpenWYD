...
// relógio das arenas (questClearTicks), a XP que o Mortal ganha por morte (a
// própria e a parte do grupo, que conta em quem recebe), pelo prêmio do Castelo e
// por cura soma no máximo o teto da faixa do nível que ele tem NO MOMENTO do
// ganho; com o dobro ligado, o teto do dobro. Passando do teto, paga até o teto e
// o resto se perde. Os valores vêm do painel (tetorodada_config.go).
//
// - Troféu (4117-4121): SEM TRAVA desde 17/09/2026, a pedido do Marco. Ele cai de
//   todo monstro como no template, o uso paga inteiro e a parte de 10% do grupo
//   também não é cortada. A primeira versão limitava o drop à metade do teto; no
//   nível 322 isso dava 7 troféus por rodada na arena dos Elfos, com "faltam N" a
//   cada um, e parava. O troféu continua do personagem: não vai ao chão nem ao
//   baú da conta (item.go), e a troca e a lojinha recusam EF_NOTRADE.
...
func (d *Dispatcher) cortaXPDaRodada(w *world.World, s *world.Session, e *world.Entity, ganho int64) int64 {
	...
	if pago < ganho && !st.avisado {
		...
		d.log.Info("teto de XP da rodada", "conta", s.AccountName, "conn", s.Conn, "nivel", e.Level,
			"total", st.total, "cortado", ganho-pago)
	}
	if d.xpDaRodada == nil {

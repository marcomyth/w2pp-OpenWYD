...
type AccountServiceClient interface {
	...
	// transaction: it is what pays for a sale in the Loja do Servidor, where the
	// seller prices an item in a currency that lives on the account, not on the
	// character. Insufficient funds come back in the body (ok=false, reason), not
	// as a gRPC error — the same shape BuyResult uses in the donate shop.
	TransferPlayerBalance(ctx context.Context, in *TransferPlayerBalanceRequest, opts ...grpc.CallOption) (*TransferPlayerBalanceResponse, error)
	...
	// ListSoldEscrowSlots returns the account's cargo slots that still hold an
	// item whose real-money listing has ALREADY BEEN SOLD.
	//
	// Selling for real money does not take the item out of the warehouse at the
	// moment the payment lands. It leaves the escrow mark (migration 0104) in the
	// slot, and a marked item is untouchable — it cannot be moved, changed,
	// refined or sold. So removing it can wait for the seller to show up, and the
	// buyer never waits for the seller: what the buyer receives comes from the
	// listing's snapshot.
	//
	// This is what the tmServer asks when the seller shows up. The slots it names
	// are to be EMPTIED, never handed back: that item was paid for and delivered
...
func (c *accountServiceClient) TransferPlayerBalance(ctx context.Context, in *TransferPlayerBalanceRequest, opts ...grpc.CallOption) (*TransferPlayerBalanceResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(TransferPlayerBalanceResponse)
	err := c.cc.Invoke(ctx, AccountService_TransferPlayerBalance_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *accountServiceClient) ListSoldEscrowSlots(ctx context.Context, in *ListSoldEscrowSlotsRequest, opts ...grpc.CallOption) (*ListSoldEscrowSlotsResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
...
type AccountServiceServer interface {
	...
	// character. Insufficient funds come back in the body (ok=false, reason), not
	// as a gRPC error — the same shape BuyResult uses in the donate shop.
	TransferPlayerBalance(context.Context, *TransferPlayerBalanceRequest) (*TransferPlayerBalanceResponse, error)
	// CreateCharacter creates a character in a free slot (_MSG_DBCreateCharacter).
...
func (UnimplementedAccountServiceServer) TransferPlayerBalance(context.Context, *TransferPlayerBalanceRequest) (*TransferPlayerBalanceResponse, error) {
	return nil, status.Error(codes.Unimplemented, "method TransferPlayerBalance not implemented")
}
...
func _AccountService_TransferPlayerBalance_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(TransferPlayerBalanceRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(AccountServiceServer).TransferPlayerBalance(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: AccountService_TransferPlayerBalance_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(AccountServiceServer).TransferPlayerBalance(ctx, req.(*TransferPlayerBalanceRequest))
	}
	return interceptor(ctx, in, info, handler)
}
...
func _AccountService_ListSoldEscrowSlots_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(ListSoldEscrowSlotsRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(AccountServiceServer).ListSoldEscrowSlots(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: AccountService_ListSoldEscrowSlots_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
...
var AccountService_ServiceDesc = grpc.ServiceDesc{
	...
	Methods: []grpc.MethodDesc{
		...
		{
			MethodName: "TransferPlayerBalance",
			Handler:    _AccountService_TransferPlayerBalance_Handler,
		},
		{

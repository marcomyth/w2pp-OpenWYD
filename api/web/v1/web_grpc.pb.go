...
type RmtWebServiceClient interface {
	...
	SavePixKey(ctx context.Context, in *SavePixKeyRequest, opts ...grpc.CallOption) (*SavePixKeyResponse, error)
...
func NewRmtWebServiceClient(cc grpc.ClientConnInterface) RmtWebServiceClient {
	return &rmtWebServiceClient{cc}
}

func (c *rmtWebServiceClient) SavePixKey(ctx context.Context, in *SavePixKeyRequest, opts ...grpc.CallOption) (*SavePixKeyResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(SavePixKeyResponse)
	err := c.cc.Invoke(ctx, RmtWebService_SavePixKey_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *rmtWebServiceClient) GetPixKey(ctx context.Context, in *GetPixKeyRequest, opts ...grpc.CallOption) (*GetPixKeyResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
...
// RmtWebServiceServer is the server API for RmtWebService service.
// All implementations must embed UnimplementedRmtWebServiceServer
// for forward compatibility.
//
// RmtWebService owns the player-to-player real-money trade surface: today the
// seller's Pix receiving key, later the listing and the charge.
...
// gated, logged or withdrawn on its own, and it names the subject in the method
// path (/web.v1.RmtWebService/SavePixKey) for anybody reading an access log.
type RmtWebServiceServer interface {
	// SavePixKey stores the account's Pix RECEIVING key — where the money goes
	// when the player sells an item for real money.
	//
	// It REFUSES while a charge is open against any of the seller's listings, and
	...
	// and take the payment in a different account. A defence that lives in the
	// screen is not a defence.
	SavePixKey(context.Context, *SavePixKeyRequest) (*SavePixKeyResponse, error)
	// GetPixKey returns the key MASKED, never whole: the form only needs to show
	// "you already registered ...1234", and a Pix key is personal payment data on a
	// public site.
...
// UnimplementedRmtWebServiceServer must be embedded to have
// forward compatible implementations.
//
// NOTE: this should be embedded by value instead of pointer to avoid a nil
// pointer dereference when methods are called.
type UnimplementedRmtWebServiceServer struct{}

func (UnimplementedRmtWebServiceServer) SavePixKey(context.Context, *SavePixKeyRequest) (*SavePixKeyResponse, error) {
	return nil, status.Error(codes.Unimplemented, "method SavePixKey not implemented")
}
...
// UnsafeRmtWebServiceServer may be embedded to opt out of forward compatibility for this service.
// Use of this interface is not recommended, as added methods to RmtWebServiceServer will
// result in compilation errors.
type UnsafeRmtWebServiceServer interface {
	mustEmbedUnimplementedRmtWebServiceServer()
}

func RegisterRmtWebServiceServer(s grpc.ServiceRegistrar, srv RmtWebServiceServer) {
	// If the following call panics, it indicates UnimplementedRmtWebServiceServer was
	// embedded by pointer and is nil.  This will cause panics if an
	// unimplemented method is ever invoked, so we test this at initialization
	// time to prevent it from happening at runtime later due to I/O.
	if t, ok := srv.(interface{ testEmbeddedByValue() }); ok {
	...
	s.RegisterService(&RmtWebService_ServiceDesc, srv)
}

func _RmtWebService_SavePixKey_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(SavePixKeyRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(RmtWebServiceServer).SavePixKey(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: RmtWebService_SavePixKey_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(RmtWebServiceServer).SavePixKey(ctx, req.(*SavePixKeyRequest))
	}
	return interceptor(ctx, in, info, handler)
}

func _RmtWebService_GetPixKey_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(GetPixKeyRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(RmtWebServiceServer).GetPixKey(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: RmtWebService_GetPixKey_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(RmtWebServiceServer).GetPixKey(ctx, req.(*GetPixKeyRequest))
	}
	return interceptor(ctx, in, info, handler)
}
...
// It's only intended for direct use with grpc.RegisterService,
// and not to be introspected or modified (even as a copy)
var RmtWebService_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "web.v1.RmtWebService",
	HandlerType: (*RmtWebServiceServer)(nil),
	Methods: []grpc.MethodDesc{
		{
			MethodName: "SavePixKey",
			Handler:    _RmtWebService_SavePixKey_Handler,
		},
		{

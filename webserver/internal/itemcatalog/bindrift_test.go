...
func TestRealCatalogTotals(t *testing.T) {
	...
	// 3242 = os 3220 históricos + as duas variantes do kit de novato (5760/5761,
	// 12/09/2026) + os três Baús do Apoiador (3304-3306, 20/09/2026) + as dez
	// Pinturas de Arma (3480-3489, 21/09/2026) + as sete Esferas de montaria
	// (2969-2975, 22/09/2026).
	if got, want := len(catalog.Items), 3242; got != want {
...
func TestBinRequirementsMatchCSV(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{661, [5]int16{219, 0, 0, 0, 0}},      // Ankh_da_Justica, issue #308 + reforma dos acessórios
		{662, [5]int16{219, 0, 0, 0, 0}},      // Ankh_da_Eternidade, issue #308 + reforma dos acessórios
		{663, [5]int16{219, 0, 0, 0, 0}},      // Ankh_da_Gloria, issue #308 + reforma dos acessórios
	} {
